...
40 PRINT "BASED ON THE ACTUAL OCCURRENCE. MOST BATTLES TEND TO RESULT"
...
101 LET I1=10*(L-W)+2
102 LET I2=10*(W-L)+2
103 LET D1=100*INT((M1*(1+(100-I1)/2000)*(1+(R1-Q1)/(R1+1))+0.5))
104 LET D2=100*INT((M2*(1+(100-I2)/2000)+0.5))
105 LET F1=5*M1/6
106 LET A1=2
107 FOR U=1 TO 4
108 PRINT
...
412 LET C5=INT(C5*(1+(1/D)*(1.28+F1/(R+1))+0.5))
414 IF C5+100/D<M1*(1+(P1-T1)/(M3+1)) THEN 424
...
430 PRINT "DESERTIONS",INT(E),INT(5+0)
432 PRINT
...
445 GO SUB800
450 IF U=1 THEN 470
460 IF C5+E<17*C2+C1/(C5+20)+5*0 THEN 490
470 PRINT "YOU LOSE ";
471 LET L=L+1
480 GOTO 555
490 PRINT "YOU WIN ";
...
580 LET T1=T1+C5+E
590 LET T2=T2+17*C2+C1/(C5+20)+5*0
600 LET P1=P1+C1
610 LET P2=P2+C2
620 LET Q1=Q1+(F+S+B)
...
676 DATA 54000,63000,10000,14400,2,4,0
678 DATA 40000,50000,10000,12000,3,5,0
680 DATA 75000,120000,5377,12853,1,6,0
...
690 DATA 65000,60000,18000,16000,2,11,0
692 DATA 37000,60000,5700,5800,2,12,0
...
930PRINT"JUNE 25-JULY 1,1862   GENERAL LEE (CSA) UPHHELD THE OFFENSIVE"
...
1180PRINT"MAY 1-4,1863    THE SOUTH HAD A COSTLY VICTORY AND LOST ONE"
