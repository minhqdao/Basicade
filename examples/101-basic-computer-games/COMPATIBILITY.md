...
The following 36 programs are currently offered by the Basicade launcher:
...
`REVERSE`, `RUSROU`, `STARS`, `TRAP`, and `WEKDAY`.
...
`QUBIC` also remains preserved in the imported source collection but is not
selectable. Its transcription contains extensive control-flow, data-table, and
strategy errors. The launcher instead offers the working `QUBIT` listing from
the BASIC Computer Games collection.
