...
250 FORX=9TO1STEP-1\CHANGEB$(X)TOA\FORY=1TO6\A(X+6*Y)=A(Y)\NEXTY\NEXTX
260 FORX=1TO6\READH$(X)\NEXTX\H$=H$(L)\GOSUB940
...
500 GOSUB640\GOSUB780\GOSUB640\GOTO270
...
700 FORX=1TOA\PRINTTAB(8*L+S);G$;G$\NEXTX\RETURN
710 IFA=1THEN720\GOTO730
720 LETA=1
730 FORX=1TOA\PRINTTAB(4*L+S);G$;G$\NEXTX\RETURN
740 FORX=1TOA\PRINTTAB(3);G$;G$\NEXTX\RETURN
...
870 PRINTTAB(7+L-X+S);G$;G$;G$\NEXTX\RETURN
...
910 PRINTTAB(7+L-X+S);G$;G$;G$\NEXTX\RETURN
920 FORX=0TO7*LSTEP(7*L)/(C+3)\PRINTTAB(3);G$;G$;TAB(X+S);G$;G$;G$;
