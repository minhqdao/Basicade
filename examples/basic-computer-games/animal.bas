...
400 Q$=A$(K)
405 Z=3:P=1
410 IF Z>LEN(Q$) THEN 420
412 IF MID$(Q$,Z,1)="\" THEN P=0
415 IF P=1 THEN PRINT MID$(Q$,Z,1);
417 Z=Z+1:GOTO 410
420 INPUT C$
430 C$=LEFT$(C$,1)
...
450 T$="\"+C$
455 X=3
460 IF X>LEN(Q$)-1 THEN 475
465 IF MID$(Q$,X,2)=T$ THEN 480
470 X=X+1:GOTO 460
475 STOP
480 Y=X+1
490 IF Y>LEN(Q$) THEN 505
495 IF MID$(Q$,Y,1)="\" THEN 510
500 Y=Y+1:GOTO 490
505 STOP
