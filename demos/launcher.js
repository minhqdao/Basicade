...
function submitInput() {
  ...
  // The interpreter's input buffer stores one byte per character, so the
  // line is normalized to printable ASCII exactly here -- the single policy
  // choke point after the faithful forwarding that fed it.
...
// ones made inside a released tap. The keyboard driver forecasts only for
// these.
...
const usesFixedShell = window.matchMedia("(pointer: coarse) and (hover: none)");
...
// then snapped it. The driver instead forecasts the final inset at focus --
// the measured height from the previous open, persisted so every open
// after the very first is exact, else a fraction of the layout -- and
// self-animates --keyboard-inset over the keyboard's own timing. The
// honest reading still arrives, but it only ever RETARGETS the running
// move or finishes it with a short glide; see terminal-keyboard.js.
// Android honors interactive-widget=resizes-content: the layout shrinks
// natively, the honest reading says 0, and the driver stands down. Desktop
// is excluded: the var is unused outside the mobile media query and a
...
// for html[data-basicade-log="native"]): Blink latches a gesture to the
...
// driven log: perfect there, untouched here. UA-gated like iosLike above
// -- the measured pan/resize mode is only known after a keyboard opens,
// but scrolling must behave from the first gesture.
const androidLike = /Android/.test(navigator.userAgent);
if (androidLike) {
  document.documentElement.dataset.basicadeLog = "native";
}
...
function insetStep(now) {
  ...
  // actually running. A pull-to-refresh pan fires visualViewport
...
function onGateMove(event) {
  ...
  if (
    shouldBlockNorthDrag({
      ...
      multiTouch: false,
      selectionCollapsed: !textSelection || textSelection.isCollapsed,
...
// oscillation in the keyboard lab.
function pinScroll() {
  if (
...
// mid-gesture). All log movement is driven here, clamped and
// pixel-rounded on every write: the ends are hard BY CONSTRUCTION.
// Momentum after a flick is emulated (native momentum needs a scroller;
// this has none). Outside the shell (desktop, touch laptops) the
// transcript is a native scroller again and the driver stands aside
// entirely -- so the browser scrolls and chains by itself. (Android
...
// chains to the outer page by itself. The driver below runs only inside
// the touch shell, where the transcript is not a scroller and every write
// is emulated. No scrollBy inside the handler: on Safari, programmatic
// page scroll during active wheel momentum fights the scrolling thread and
// the page wiggles. Line/page delta modes are converted to pixels
// (decideLogWheel).
screen.addEventListener(
  "wheel",
  (e) => {
...
// The interpreter's ASCII policy applies exactly once, at submit
// (toEngineText).
 ...
 * upper-cased for the terminal and capped at the interpreter's line length.
 * @returns {string}
 */
