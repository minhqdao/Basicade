...
/*                                                               */
/* Version 3.30+ not going to be mentioned here anymore          */
/*                                                               */
/*---------------------------------------------------------------*/
...
static char *Banner[] = {
  ...
  "                                    ########  ##     ##  ######  ####  ###### ",
  "                                                                              ",
  "Bywater BASIC Interpreter, version 3.40                                       ",
  "Copyright (c) 1993, Ted A. Campbell                                           ",
...
break_mes (int x /* Parameter 'x' is never used */ )
{
  ...
  else
  {
    ...
    if (My->CurrentVersion->OptionVersionValue & (C77))
    {
      ...
      {
        fprintf (My->SYSPRN->cfp, "FILE:%s, ", My->ProgramFilename);
      }
    }
    fprintf (My->SYSPRN->cfp, "Program interrupted");
    if (My->ThisLine)                /* break_mes */
    {
      ...
      {
        fprintf (My->SYSPRN->cfp, " at line %d", My->ThisLine->number);        /* break_mes */
      }
    }
    fputc ('\n', My->SYSPRN->cfp);
    ResetConsoleColumn ();
    fflush (My->SYSPRN->cfp);
  }
  break_handler ();
}
...
execute_program (char *FileName)
{
  ...
  {
    fprintf (My->SYSPRN->cfp, "Failed to open file %s\n", FileName);
  ...
  if (!My->IsCheckOnly) bwb_RUN (My->StartMarker);
}
...
main (int argc, char **argv)
{
  ...
  {
    /* no parameters */
    /* and no options! -- should probably find better way to do this */
    if (My->IsInteractive && !My->IsSilent)
        /* !IsSilent is actually redundant here (but shouldn't be) */
    {
      bwx_signon ();
    }
    else
  ...
  for (i = 1; i < argc; i++)
  {
          ...
          || bwb_stricmp (argv[i], "/t") == 0)
    {
      ...
      if (bwb_stricmp (argv[i], "--check") == 0
          || bwb_stricmp (argv[i], "-c") == 0
          || bwb_stricmp (argv[i], "/check") == 0
          || bwb_stricmp (argv[i], "/c") == 0)
    {
        My->IsCheckOnly = TRUE;
    }
    else
      if (bwb_stricmp (argv[i], "--silent") == 0
          || bwb_stricmp (argv[i], "-s") == 0
          || bwb_stricmp (argv[i], "/silent") == 0
          || bwb_stricmp (argv[i], "/s") == 0)
    {
        My->IsSilent = TRUE;
    }
    else
      if (bwb_stricmp (argv[i], "--help") == 0
          || bwb_stricmp (argv[i], "--?") == 0
          || bwb_stricmp (argv[i], "-h") == 0
          || bwb_stricmp (argv[i], "-?") == 0
          || bwb_stricmp (argv[i], "/help") == 0
          || bwb_stricmp (argv[i], "/h") == 0
          || bwb_stricmp (argv[i], "/?") == 0)
    {
       fputs("SYNTAX:  bwbasic [ options ] [ program.bas ]\n", My->SYSOUT->cfp);
       fputs("OPTIONS:\n"
             "\t--profile|-p filename:    specify alternative to profile.bas\n"
             "\t--tape|-t tapefile.inp:   load program \"tape\" tapefile.inp\n"
             "\t--silent|-s:              suppress all prompts and banner\n"
             "\t--check|-c:               check syntax of program, "
                                          "but do not execute\n"
             "\t--help|-h:                print this message and exit\n"
             "\t--?|-?:                   alias for --help\n",
             My->SYSOUT->cfp);
       fputs("(a '/' character may be substituted for '--' or '-' "
             "to indicate options)\n", My->SYSOUT->cfp);
        exit(0);
    }
    else
...
bwb_fload (char *FileName)
{
  ...
  while (fgets (tbuf, tlen, file))        /* bwb_fload */
  {
    ...
    else if (bwb_strnicmp (tbuf, Magic_Word, Magic_Length) == 0)
    {
      ...
      {
        fprintf (My->SYSPRN->cfp, "%s Filename\n", Magic_Word);
        ResetConsoleColumn ();
        return FALSE;
      }
      ...
      if (Result == FALSE)
      {
        fprintf (My->SYSPRN->cfp, "%s %s Failed\n", Magic_Word, varname);
        ResetConsoleColumn ();
        return FALSE;
      }
    }
...
bwb_ladd (char *buffer, LineType * p, int IsUser)
{
  ...
  {
    /* ERROR */
    fprintf (My->SYSPRN->cfp, "UNBALANCED QUOTES: %s\n", buffer);
    ResetConsoleColumn ();
  ...
  do
  {
    ...
    if (is_empty_string (ThisStatement) == FALSE)
    {
      ...
      {
        /* ERROR */
        fprintf (My->SYSPRN->cfp, "Out of memory\n");
        ResetConsoleColumn ();
      ...
      if (buff_read_line_number (newbuffer, &(l->position), &l->number))
      {
        ...
        {
          /* ERROR */
          fprintf (My->SYSPRN->cfp, "%d < %d - LINE OUT OF ORDER: %s\n",
        ...
        {
          /* ERROR */
          fprintf (My->SYSPRN->cfp, "INVALID LINE NUMBER: %s\n", buffer);
          ResetConsoleColumn ();
        ...
        {
          /* ERROR */
          fprintf (My->SYSPRN->cfp, "MISSING SPACE AFTER LINE NUMBER: %s\n",
                   buffer);
          ResetConsoleColumn ();
...
bwb_execline (void)
{
  ...
  if (My->IsTraceOn == TRUE)
  {
    ...
    {
      fprintf (My->SYSPRN->cfp, "[ %d %s ]", l->number, l->buffer);
    }
  }
             ...
             StackItem = StackItem->next)
        {
          ...
          if (l != NULL)
          {
            if (MINLIN <= l->number && l->number <= MAXLIN)
            {
              ...
              if (l->buffer == NULL)
              {
                fprintf (My->SYSPRN->cfp, "%d\n", l->number);
              }
              else
              {
                fprintf (My->SYSPRN->cfp, "%d:%s\n", l->number, l->buffer);
              }
            }
          }
        }
